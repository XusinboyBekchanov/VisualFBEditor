...
Function MariaDBBox.Open(ByRef FileName As WString, ByRef Password As WString = "") As Boolean
	If FSQLite3 Then mysql_close(FSQLite3)
	FSQLite3 = 0
	If FSQLite3Mem Then mysql_close(FSQLite3Mem)
	FSQLite3Mem = 0
...
Function MariaDBBox.MemOpen(sFileName As UString, Password As UString = "", Synchronization As Boolean = 0) As Boolean
	If FSQLite3 Then mysql_close(FSQLite3)
	FSQLite3 = 0
...
Function MariaDBBox.MemSave() As Boolean
...
Sub MariaDBBox.Close()
	...
	End If
End Sub
Function MariaDBBox.GetSQLitePtr(Index As Long = 0) As sqlite3 Ptr
	ErrStr = ""

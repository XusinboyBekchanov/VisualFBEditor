...
'#Region "Form"
	...
	Constructor frmImageListEditor
		' frmImageListEditor
		With This
			.Name = "frmImageListEditor"
			.Text = ML("ImageList Editor")
			.Designer = @This
			.OnShow = @Form_Show_
			.Margins.Top = 0
			.Margins.Right = 0
		...
		With lvImages
			...
			.Columns.Add ML("Key"), , 100
			.Columns.Add ML("Resource name / Path"), , 130
	...
	Dim Shared fImageListEditor As frmImageListEditor
	pfImageListEditor = @fImageListEditor
	
'#End Region

Private Sub frmImageListEditor.Form_Show_(ByRef Sender As Form)
	*Cast(frmImageListEditor Ptr, Sender.Designer).Form_Show(Sender)
End Sub
Private Sub frmImageListEditor.Form_Show(ByRef Sender As Form)
...
'				lvImages.ListItems.Item(lvImages.ListItems.Count - 1)->Text(2) = FilePath
	...
	Items = QWString(Des->ReadPropertyFunc(CurrentImageList, "Items"))
	For i As Integer = 0 To Items.Count - 1
		
	Next
End Sub

Private Sub frmImageListEditor.cmdCancel_Click_(ByRef Sender As Control)
	*Cast(frmImageListEditor Ptr, Sender.Designer).cmdCancel_Click(Sender)
End Sub
Private Sub frmImageListEditor.cmdCancel_Click(ByRef Sender As Control)
	ModalResult = ModalResults.Cancel
	Me.CloseForm
End Sub

Private Sub frmImageListEditor.cmdOK_Click_(ByRef Sender As Control)
	*Cast(frmImageListEditor Ptr, Sender.Designer).cmdOK_Click(Sender)
End Sub
Private Sub frmImageListEditor.cmdOK_Click(ByRef Sender As Control)
...
Private Sub frmImageListEditor.lvImages_ItemActivate_(ByRef Sender As ListView, ByVal ItemIndex As Integer)
	*Cast(frmImageListEditor Ptr, Sender.Designer).lvImages_ItemActivate(Sender, ItemIndex)
End Sub
Private Sub frmImageListEditor.lvImages_ItemActivate(ByRef Sender As ListView, ByVal ItemIndex As Integer)
...
Private Sub frmImageListEditor.lvImages_SelectedItemChanged_(ByRef Sender As ListView, ByVal ItemIndex As Integer)
	*Cast(frmImageListEditor Ptr, Sender.Designer).lvImages_SelectedItemChanged(Sender, ItemIndex)
End Sub
Private Sub frmImageListEditor.lvImages_SelectedItemChanged(ByRef Sender As ListView, ByVal ItemIndex As Integer)
...
Private Sub frmImageListEditor.tbToolbar_ButtonClick_(ByRef Sender As ToolBar,ByRef Button As ToolButton)
	*Cast(frmImageListEditor Ptr, Sender.Designer).tbToolbar_ButtonClick(Sender, Button)
End Sub
Private Sub frmImageListEditor.tbToolbar_ButtonClick(ByRef Sender As ToolBar,ByRef Button As ToolButton)
...
Private Sub frmImageListEditor.Form_Create_(ByRef Sender As Control)
	*Cast(frmImageListEditor Ptr, Sender.Designer).Form_Create(Sender)
End Sub
Private Sub frmImageListEditor.Form_Create(ByRef Sender As Control)
	pfPath->cboType.Clear
	pfPath->cboType.AddItem "Resource"
	pfPath->cboType.AddItem "Path"
End Sub

...
`IPAddress` - `IPAddress` is a Control within the MyFbFramework, part of the freeBasic framework. <br> `IPAddress` - An Internet Protocol (IP) address control allows the user to enter an IP address in an easily understood format.
...
|[`Clear`]("IPAddress.Clear.md")|Resets all fields to zero values|
...
|[`ReadProperty`]("IPAddress.ReadProperty.md")|Loads IP configuration from stream|
...
|[`WriteProperty`]("IPAddress.WriteProperty.md")|Saves IP configuration to stream|
## Events
|Name|Description|
| :------------ | :------------ |
|[`OnChange`]("IPAddress.OnChange.md") |Triggered when any address field is modified|
...
|[`OnFieldChanged`]("IPAddress.OnFieldChanged.md") |Raised when specific octet changes|

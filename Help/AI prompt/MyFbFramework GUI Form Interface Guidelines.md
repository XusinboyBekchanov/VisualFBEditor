...
ComboBoxEx: ComboBoxEx is an enhanced version of ComboBoxEdit, inheriting its core functionality while adding advanced features such as the ability to display images for each item.
...
NumericUpDown: Represents a spin box (also known as an up-down control) that displays numeric values.
OpenFileControl: Represents a box for choosing files.
PrintPreviewControl: Represents the raw preview part of print previewing, without any dialog boxes or buttons.
...
SearchBox: A subclass of TextBox that has been tailored for use as a search textbox.
...
ToolTips: An element in which, when hovering over a marker or region element or component, a text box displays information about that element.
...
HorizontalBox: Arranges items in a horizontal row.
PagePanel: Used to organize controls on hidden panels.
...
ScrollControl: A container that makes its child scrollable.
...
VerticalBox: Arranges items in a vertical column.
...
HTTPConnection: Constructs a connection to the host (port) as given in the url.
...
MariaDBBox: A container that provides a pre-installed and configured environment for working with the MariaDB database.
PopUpMenu: Represents a context menu.
PrintDocument: Defines a reusable object that sends output to a printer, when printing.
Printer: Enables you to communicate with a system printer (initially the default printer).
SQLite3Component: A component that facilitates integration with the SQLite3 database.
...
### Project File Structure
    VisualFBEditor and MyFbFramework support editing project files.
